...
class FilmReviewFromAllRequest(private val context: Context, private val filmID: Int) {
    fun sendRequest(page: Int, callback: FilmReviewFromAllRequestCallback) {
        val requestURL = "${PopularinAPI.FILM}$filmID/reviews/from/all?page=$page"

        val filmReviewFromAll = object : JsonObjectRequest(Method.GET, requestURL, null, Response.Listener { response ->
