        ...
        mUserFollowerRequest.sendRequest(page, object : UserFollowerRequest.Callback {
            override fun onSuccess(totalPage: Int, userList: ArrayList<User>) {
                when (mIsLoadFirstTimeSuccess) {
                    ...
                    false -> {
                        ...
                        val insertIndex = mUserList.size
                        mUserList.addAll(insertIndex, userList)
                        mUserAdapter = UserAdapter(mContext, mUserList, this@FollowerFragment)
            ...
            override fun onNotFound() {
                ...
                mTextMessage.text = when (isSelf) {
                    true -> R.string.empty_self_follower.toString()
                    false -> R.string.empty_user_follower.toString()
                }
            }
